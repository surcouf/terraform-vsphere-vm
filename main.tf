...
resource "local_file" "ssh_public_key" {
  filename  = pathexpand("~/.ssh/ssh_id_${var.vmname}.pub")
  content   = trimspace(tls_private_key.admin.public_key_openssh)
}

resource "local_sensitive_file" "ssh_key" {
  filename  = pathexpand("~/.ssh/ssh_id_${var.vmname}")
...
resource "vsphere_virtual_machine" "vm" {
  ...
  extra_config            = merge({
      ...
      "guestinfo.metadata" = base64encode(yamlencode({
          instance-id     = "${var.staticvmname != null ? var.staticvmname : format("${var.vmname}${var.vmnameformat}", count.index + var.vmstartcount)}"
          local-hostname  = "${var.staticvmname != null ? var.staticvmname : format("${var.vmname}${var.vmnameformat}", count.index + var.vmstartcount)}"
          hostname        = "${var.staticvmname != null ? var.staticvmname : format("${var.vmname}${var.vmnameformat}", count.index + var.vmstartcount)}"
          network         = {
            version = 1
            config  = [ 
  ...
  connection {
    ...
    port        = var.ssh_port
    private_key = file(pathexpand("~/.ssh/ssh_id_${var.staticvmname != null ? var.staticvmname : var.vmname}") )
  }
...
resource "ansible_host" "vm" {
  ...
  variables   = {
    ...
    ansible_ssh_private_key_file  = pathexpand("~/.ssh/ssh_id_${var.staticvmname != null ? var.staticvmname : var.vmname}")
